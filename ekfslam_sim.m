...
% setup plots
fig=figure;
plot3(lm(1,:),lm(2,:),lm(3,:),'b*')
hold on, axis equal
plot3(wp(1,:),wp(2,:),wp(3,:), 'g', wp(1,:),wp(2,:),wp(3,:),'g.')
...
T= 0; % initial azimuth angle
Ps= 0; % initial elevation angle
...
while iwp ~= 0
    ...
    % compute true data
    [T,iwp]= compute_steering(xtrue, wp, iwp, AT_WAYPOINT, T, RATET, MAXT, dt);
    if iwp==0 & NUMBER_LOOPS > 1, iwp=1; NUMBER_LOOPS= NUMBER_LOOPS-1; end % perform loops: if final waypoint reached, go back to first
    xtrue= vehicle_model(xtrue, V,T, WHEELBASE,dt);
    [Vn,Tn]= add_control_noise(V,T,Q, SWITCH_CONTROL_NOISE);
    
    % EKF predict step
    [x,P]= predict (x,P, Vn,Tn,QE, WHEELBASE,dt);
